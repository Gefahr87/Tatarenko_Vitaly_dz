...
def check_1(lst_obj):
    ...
    Сложность: О(len(lst_obj)), т.е O(n) - линейная
    """
    lst_to_set = set(lst_obj)  # О(len(lst_obj) т.е. О(len(n)) n + n
...
def check_2(lst_obj):
    ...
    """
    for j in range(len(lst_obj)):          # O(n), n + n + n
...
def check_3(lst_obj):
    ...
    for i in range(len(lst_obj) - 1):        # O(n), n + n
